...
Local nValCont	:= 0
Local cKeySD1	:= xFilial('SD1')+nNFiscal+cSerie+cFornece+cLoja                                                         
Local cPerICMS	:= SD1->D1_PICM       
Local nPerRed 	:= Posicione("SF4", 1, xFilial("SF4")+cTES,"F4_PICMDIF")   


If nOpcao == 3 .AND. cTES $ "134/084"
	...
	If SF3->( DbSeek(xFilial("SF3")+cEntrada+nNFiscal+cSerie+cFornece+cLoja) )
		...
		SF3->(MsUnLock())
	EndIf
	ApMsgInfo("Ol�, devido esta ser uma nota de importa��o os valores cont�beis dos livros foram ajustados para respeitar o regime especial da empresa. Valor Atual: "+cValToChar(nValCont),"Team Tex - Reg. Especial")
