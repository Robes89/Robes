		...
		While (cAliasReg)->( ! Eof() ) 
			...
			While (cAliasReg)->( ! Eof() ) .And. cPed = (cAliasReg)->C2_NUM
					...
					While (cAliasReg)->( ! Eof() ) .AND. cOp = (cAliasReg)->(C2_NUM + C2_ITEM + C2_SEQUEN)
							...
							cImp11  := Subst(Alltrim((cAliasReg)->B1_DESC),Val(alltrim(Subst((cAliasReg)->B1_CPOSDESC,1,3))),;							
						    Val(alltrim(Subst((cAliasReg)->B1_CPOSDESC,4,3)))-LEN(ALLTRIM((cAliasReg)->B1_DESC)))  
