...
WSRESTFUL estoque DESCRIPTION "Integração com Vtrina"
...
WSMETHOD GET DESCRIPTION "Busca estoque dos produtos" WSSYNTAX "/estoque"
WSMETHOD PUT DESCRIPTION "Confirma atualização do estoque" WSSYNTAX "/estoque/{id}"

END WSRESTFUL

WSMETHOD GET WSRECEIVE page, limit, marketplace, productId, variantId, codigo, codbar, dataAtu, horaAtu WSSERVICE estoque
...
conout("ESTOQUE - Empresa/Filial: " +cEmpAnt+"/"+cFilAnt)
...
BeginSQL alias cAlias
	SELECT	SB1.B1_COD, SB1.B1_DESC, SB1.B1_CODBAR, ZDD.ZDD_VARID,
			...
			COALESCE(SB2.B2_RESERVA+SB2.B2_QEMP+SB2.B2_QACLASS+SB2.B2_QPEDVEN,0) AS RESERVA,
			...
			ZDD.R_E_C_N_O_ AS ZDD_RECNO, ZDD.ZDD_QUANT
...
If (cAlias)->(!Eof())
	While (cAlias)->(!Eof())
		...
		::SetResponse('"estoqueDisponivel": ' + ALLTRIM(STR((cAlias)->ZDD_QUANT)) + ', ')
...
WSMETHOD PUT WSSERVICE estoque

Local lPut := .T.

// Exemplo de retorno de erro
If Len(::aURLParms) == 0
	SetRestFault(400, "É obrigatório informar o id do estoque")
	lPut := .F.
Else
	nRecno := VAL(::aUrlParms[1])

	ConOut("[ESTOQUE] - Confirmacao de estoque recebido no Vtrina - Empresa/Filial: " +cEmpAnt+ "/" +cFilAnt+ " ("+Time()+")")
	...
	::SetResponse('"Estoque atualizado"')
EndIf
