...
//TRYEXCEPTION
	...
	If ValType(OPARSE:_SOAP_ENV_ENVELOPE:_SOAP_ENV_BODY:_NS1_CUSTOMERADDRESSLISTRESPONSE:_RESULT:_ITEM) == "A"
		For _nX := 1 To Len(OPARSE:_SOAP_ENV_ENVELOPE:_SOAP_ENV_BODY:_NS1_CUSTOMERADDRESSLISTRESPONSE:_RESULT:_ITEM)
			//If AllTrim(OPARSE:_SOAP_ENV_ENVELOPE:_SOAP_ENV_BODY:_NS1_CUSTOMERADDRESSLISTRESPONSE:_RESULT:_ITEM[_nX]:_IS_DEFAULT_SHIPPING:TEXT) == "true"
				...
				_cAuxEnd 	:= AllTrim(OPARSE:_SOAP_ENV_ENVELOPE:_SOAP_ENV_BODY:_NS1_CUSTOMERADDRESSLISTRESPONSE:_RESULT:_ITEM[_nX]:_STREET:TEXT)
	...
	Else
		...
		_cAuxEnd 	:= AllTrim(OPARSE:_SOAP_ENV_ENVELOPE:_SOAP_ENV_BODY:_NS1_CUSTOMERADDRESSLISTRESPONSE:_RESULT:_ITEM:_STREET:TEXT)
		aAdd(aDestinos,{_cID, _cTel, _cCEP, _cAuxEnd})
	EndIf
	...
	If ValType(oBill) == "O"
		...
		_cAuxEnd := Alltrim(oBill:_STREET:TEXT)
	...
	//ConOut( VarInfo("aDestinos",aDestinos))
		...
		For _nX := 1 To Len(aDestinos)
			...
			aEnd := StrTokArr(aDestinos[_nX,4],CRLF)
		
			cEnd1   := aEnd[1]
			cNum1   := aEnd[2]
			cBairr  := Iif(Len(aEnd)>3,aEnd[4],aEnd[3])
			cComple := Iif(Len(aEnd)>2,aEnd[3],"")
			cComple := Iif(cComple == cBairr, "", cComple)

			_cAuxEnd := ENDAUXLIA(FwCutOff(aDestinos[_nX,4]))
			If oLoc <> NIL
				_cEst 		:= AllTrim(UPPER(NOACENTO(DecodeUtf8(oLoc:uf))))
				_cEst 		:= AllTrim(POSICIONE("SX5",1,xFilial("SX5")+"12"+_cEst,"X5_DESCRI"))
				If !Empty(oLoc:logradouro)
					_cAuxEnd 	:= AllTrim(STRTRAN(AllTrim(UPPER(NOACENTO(DecodeUtf8(oLoc:logradouro)))),AllTrim(_cAuxEnd),"")) + ", " + AllTrim(_cAuxEnd)
				Else
					_cAuxEnd    := cEnd1 + ", " + cNum1
				Endif
				...
				Endif
				AADD(_aDados,{"A1_END"		,Upper(_cAuxEnd)        							,NIL})
				_cComplem := UPPER(NOACENTO(COMPLEAUX(_cComplem)))
				_cComplem := StrTran(_cComplem,AllTrim(UPPER(NOACENTO(DecodeUtf8(oLoc:bairro)))),"")
				AADD(_aDados,{"A1_COMPLEM"	,_cComplem											,NIL})
				*/
				If !Empty(cComple)
					AADD(_aDados,{"A1_COMPLEM"	,cComple											,NIL})
				Endif
				If !Empty(oLoc:bairro)
					AADD(_aDados,{"A1_BAIRRO"	,AllTrim(UPPER(NOACENTO(DecodeUtf8(oLoc:bairro))))	,NIL})
				Else
					AADD(_aDados,{"A1_BAIRRO"	,AllTrim(UPPER(NOACENTO(DecodeUtf8(cBairr))))	,NIL})
				Endif
