	...
	While (cAlsQry)->(!EOF())
		...
		cJson := "{"
			...
			cJson += "[{"
				...
				cJson += '"weight":' + cValToChar(_nPeso) + ','
				cJson += '"width":' + cValToChar(_nLarg) + ','
				cJson += '"height":"' + AllTrim(STR(_nAltu)) + '",'
				cJson += '"length":"' + AllTrim(STR(_nComp)) + '",'
				...
				cJson += "{"
					...
					cJson += '"invoice_total_value":' + cValToChar(SF2->F2_VALBRUT) + ','
					cJson += '"invoice_products_value":' + cValToChar(_nValPrd) + ','
