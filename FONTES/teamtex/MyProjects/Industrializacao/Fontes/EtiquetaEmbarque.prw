...
User Function EtEmbTri()
	...
	If Pergunte("TRYETTEAM",.T.)
		...
		cQuery += "	AND	C2_EMISSAO	BETWEEN '" + DtoS(MV_PAR09) + "' AND '" + DtoS(MV_PAR10) + "' "
		...
		While (cAliasReg)->( ! Eof() )
			...
			While (cAliasReg)->( ! Eof() ) .AND. cOp=(cAliasReg)->(C2_NUM+C2_ITEM+C2_SEQUEN) 
				...
				cQuery := " SELECT SUM(SD4.D4_QUANT) AS NVOLUME FROM " + RetsqlName("SD4") + "  SD4 "
				...
				For nX := 1 to nQtd
					...
					cText += "1911S0100050100P009P009" + Alltrim(Substr(cImpBAR,13,1)) + CRLF    //1+ CRLF
