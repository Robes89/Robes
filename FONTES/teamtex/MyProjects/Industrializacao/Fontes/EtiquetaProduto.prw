...
User Function EtProTri()
	...
	If Pergunte("TRYETTEAMP",.T.)
		...
		cQuery += " JOIN " + RetsqlName("SC5") + " SC5 ON SC5.C5_FILIAL=SC2.C2_FILIAL AND SC5.C5_NUM=SC2.C2_NUM"
		cQuery += " JOIN " + RetsqlName("SA1") + " SA1 ON SA1.A1_COD = SC5.C5_CLIENTE AND SA1.A1_LOJA = SC5.C5_LOJACLI "
		...
		cQuery += "	AND SA1.A1_COD BETWEEN '" + MV_PAR05+ "' AND '" + MV_PAR06+ "' " 
		...
		cQuery += "	AND	SC2.D_E_L_E_T_ = ''  AND SC5.D_E_L_E_T_='' AND SB1.D_E_L_E_T_ = '' AND SA1.D_E_L_E_T_ = '' AND SD3.D_E_L_E_T_ = '' "		
		...
		While (cAliasReg)->( ! Eof() )
			...
			cDescProd    := Substr(Alltrim((cAliasReg)->B1_DESC),1,30)

			cModelo:=Posicione("SX5",1,xFilial("SX5")+"ZX"+(cAliasReg)->B1_CODTYPE,"X5_DESCRI")		
			...
			If Select(cAliasRegSDB) > 0
				cAliasRegSDB->( DbCloseArea() )
			EndIf

			DBUseArea( .T. , "TOPCONN" , TcGenQry(,,cQuery) , cAliasRegSDB )

			dbSelectArea(cAliasRegSDB)

			While (cAliasRegSDB)->(!Eof())	

				cLote        := Alltrim((cAliasRegSDB)->DB_LOTECTL)  
				cImpBARSerie := PadL(Alltrim((cAliasRegSDB)->DB_NUMSERI),13,'0')//Alltrim((cAliasRegSDB)->DB_NUMSERI)
				...
				cText += "R0000"+ CRLF
				cText += "A2"+ CRLF
				cText += "1f6303701430050" + cImpBAR + CRLF
				cText += "ySU8"+ CRLF
				...
				cText += "1911S0101330037P009P009" + Alltrim(Substr(cImpBAR,13,1)) + CRLF    //1+ CRLF
				...
				cText += "1911S0101840051P006P006" + cDescProd + CRLF
				...
				cText += "1911S0101140086P008P009" + cModelo + CRLF
				cText += "1911S0100960073P008P009" + cGrupo + CRLF
				...
				cText += "1911S0100120075P009P009" + Alltrim(Substr(cImpBARSerie,1,1)) + CRLF    //1+ CRLF
				...
				cText += "1911S0100120055P009P009" + Alltrim(Substr(cImpBARSerie,13,1)) + CRLF    //1+ CRLF
				cText += "Q0001"+ CRLF
				cText += "E"+ CRLF
