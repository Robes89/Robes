...
Local nValtrib  := 0
Local nCountIT  := 0
Local lAux          := .F. //especifico TEAM TEX
...
DbSelectArea ("SX6")
SX6->(DbSetOrder (1))
If (SX6->(DbSeek (cFilant+"MV_SUBTRIB")))
    Do While !SX6->(Eof ()) .And. cFilant==SX6->X6_FIL .And. "MV_SUBTRIB"$SX6->X6_VAR
        If !Empty(SX6->X6_CONTEUD)
            cMVSUBTRIB += "/"+AllTrim(SX6->X6_CONTEUD)
        EndIf
        SX6->(DbSkip ())
    EndDo
ElseIf (SX6->(DbSeek (SPACE(LEN(SX6->X6_FIL))+"MV_SUBTRIB")))
    Do While !SX6->(Eof ()) .And. "MV_SUBTRIB"$SX6->X6_VAR
        If !Empty(SX6->X6_CONTEUD)
            cMVSUBTRIB += "/"+AllTrim (SX6->X6_CONTEUD)
        EndIf
        SX6->(DbSkip ())
...
IF ALLTRIM(GetNewPar("MV_CMPUSR",""))  <>  ""
...
If cTipo == "1"
    ...
    For nCountNF := 1 To Len(aCupRefLoj)
        ...
        If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
            ...
            Else
                ...
                If !SF2->F2_TIPO $ "DB" 
                    ...
                    /* Futura implementa��o - Campos de cliente de retirada inexistentes no dicion�rio da 12
        ...
        //                      If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
                        ...
                        // ESPECIFICO TEAM TEX
                        // GRAVAR O PESO CUBICO NA DANFE
                        // MARCOS BIDO - 25/05                                                   
                        
                        If !lAux // Processar somente 1 vez
                                          
                            If !Empty(SC5->C5_PCUB01) .OR. !Empty(SC5->C5_PCUB02)//Se houver valor de m3 ou peso cubado imprime a mensagem

                                cMensCli += "M3 = " + Alltrim(Transform(SC5->C5_PCUB01,PesqPict("SB5","B5_XMETCUB"))) + " / PESO CUBADO = " + Alltrim(Transform(SC5->C5_PCUB02,PesqPict("SB5","B5_XPESCUB"))) + " - "  
        
                            endif
                            lAux := .t.
                        endif
                        // fim do especifico TEAM TEX
                      ...
                      //Tratamento para que o valor de PIS ST,COFINS ST venha a compor o valor total da nota.
                        ...
                        If (lCalSol .OR.  lMVCOMPET .OR. lDifal )
                            ...
                            If MsSeek(xFilial("SF3")+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE)
                                If At (SF3->F3_ESTADO, ALLTRIM(cMVSUBTRIB))>0
                                    nPosI   :=  At (SF3->F3_ESTADO, ALLTRIM(cMVSUBTRIB))+2
                                    nPosF   :=  At ("/", SubStr (ALLTRIM(cMVSUBTRIB), nPosI))-1
                                    nPosF   :=  IIf(nPosF<=0,len(ALLTRIM(cMVSUBTRIB)),nPosF)
                                    aAdd (aIEST, SubStr (ALLTRIM(cMVSUBTRIB), nPosI, nPosF))    //01 - IE_ST
            ...
            If FieldPos("F1_TRANSP") > 0 .And. !Empty(SF1->F1_TRANSP)
                ...
                aadd(aTransp,SA4->A4_EMAIL  )
                    If !Empty(SF1->F1_PLACA)
                        ...
                        aadd(aVeiculo,"")//RNTC
                    EndIf       
                //Campos customizados, buscam informa��es digitadas na nota ESPECIFICO TEAMTEX
                //Autor: Daniel Castro - TOTVS
                //Data: 05/09/17
                aadd(aTransp,SF1->F1_XMARCA)
                aadd(aTransp,SF1->F1_XNUMERA)   
            EndIf
            ...
            While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
               ...
               EndIf        
                ...
                aadd(aProd, {Len(aProd)+1,;  
                    ...
                    nValIFecp ,; //aprod[43]  Valor do FECP. 
                    cCodlan,; //aprod[44]  C�digo de Benef�cio Fiscal na UF aplicado ao item .
                    IIf(SB5->(ColumnPos("C")) > 0,IIf(Val(SB5->B5_2CODBAR)==0,"",StrZero(Val(SB5->B5_2CODBAR),Len(Alltrim(SB5->B5_2CODBAR)),0)),""),; //aprod[45]  C�digo de barra da segunda unidade de medida.
                    IIf(SB1->(ColumnPos("B1_CODGTIN")) > 0,SB1->B1_CODGTIN,""),;
                  })
