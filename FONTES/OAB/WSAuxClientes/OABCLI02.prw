...
WsMethod Post WsReceive cCgcEmp WsService POSTLICENC
	...
	//Efetuando conex�o de acordo com CGC enviado na entrada
	RpcSetEnv("01","01")
...
/* -----I-------------*/Case Empty(cDataFim)//Inclus�o///////////////////////////////
						...
						If lInclui //s� ser� verdadeira caso s� exista cancelamentos ja finalizados
...
WsRestFul POSTISENC Description "Isencao"
WsData cCgcEmp		As String
WsMethod Post Description "Isencao" WsSyntax "/POSTISENC"
End WsRestFul

WsMethod Post WsReceive cCgcEmp WsService POSTISENC
	...
	If len(oJsoAux:Isencao) > 0
		...
		//Cria servi�o no montitor
		aCriaServ := U_MonitRes("000002", 1, Len(oJsoAux:Isencao) )   
		cIdPZB 	  := aCriaServ[2]

		::SetContentType("application/json")
		cJson += '"Licenciamento":['

		For nX := 1 to len(oJsoAux:Isencao)

			cCodCli  := oJsoAux:Isencao[nX]:CodigoCli 
			cCodOco  := oJsoAux:Isencao[nX]:CodOcorrencia 
			cOcorr   := oJsoAux:Isencao[nX]:DescOcorrencia 
			cOcorr   := oJsoAux:Isencao[nX]:Isencao
			cDataIn  := oJsoAux:Isencao[nX]:DataInicio 
			cDataFim := oJsoAux:Isencao[nX]:DataFim 
			lInclui  := .T.
			lAltera  := .F.
