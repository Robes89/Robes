...
//Ajusta os perguntes
	AjusSx1()
IF LockbyName("U_V5FAT006"+__cUserId  , .F., .F.)
	lV5 := .T.
...
Else
	If !lBlind
		MsgStop( 'Usuario em acesso com a rotina !!! ' )
	Else
		ConOut( 'V5FAT006 - Usuario em acesso com a rotina.' )
		RpcClearEnv()
	EndIf
EndIf
if lV5
	UnLockbyName("U_V5FAT006"+__cUserId , .F., .F.)
EndIf
Return
	...
	If ( Alltrim(SF2->F2_OK) == cMark )
		...
		If SF2->F2_P_AGLUT == "S"
			...
			If V5FAT006->(!EOF())
				...
				If Len(_aAnexos)
					...
					If ! CpyT2S( cFileAglut , cDirAglut ,.T. ) .OR. TamFile(cFileAglut) < 20024
  					   SF2->(dbGoTo(nRegSF2))
					   SF2->(DbSkip())
		...
		If !Empty( cAnexo ) .And. !lPreview
			...
			If SA1->A1_XBOLFEB == "S"
				...
				cPar03 := SA1->A1_COD
				cPar04 := SA1->A1_LOJA
				cPar05 := IIF(SA1->A1_XBOLUNI == "S",SF2->F2_XID, SF2->F2_DOC)
				cPar06 := IIF(SA1->A1_XBOLUNI == "S",SF2->F2_XID, SF2->F2_DOC)
...
Static Function EnviaEma(cEmail,cSubject,cTo,cAnexos)
	*--------------------------------------------------------------*
	...
	For nU := 1 to Len(aANexos)
		if TamFile(aANexos[nU]) < 5024 .and. !( 'TXT' $ aANexos[nU]  )
			RETURN .F.
		EndIf
	Next	
