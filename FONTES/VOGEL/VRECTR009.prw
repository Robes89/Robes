...
Static Function VRECONT9(cArquivo)
	...
	// Posicionamento no contrato que será revisado.
		IF cContra <> aDados[i,2] 
			...
			If CN9->( dbSeek(cFil + PadR(cContra,TamSX3("CN9_NUMERO")[1]) + "05") )	
				...
				//oModel:GetModel('CN9MASTER'):SetProperty("CN9_TIPREV",MODEL_FIELD_VALID,MTBlcVld("CN9","CN9_TIPREV"	,"ExistCpo('CN0', c)",.F.,.T.) )
				...
				oModel:SetValue( 'CN9MASTER'    , 'CN9_JUSTIF'  , 'EFICIENCIA TRIBUTARIA' )    // É obrigatório o preenchimento da justificativa de revisão do contrato.
				oModel:SetValue( 'CN9MASTER'    , 'CN9_XOBS'  , 'EFICIENCIA TRIBUTARIA' ) 
			Else
			 	Loop
			EndIF	
		EndIF
		...
		EndIf
		cQryCNB := " SELECT "
		cQryCNB += " 	CNB_CONTRA, CNB_NUMERO, MAX(CNB_REVISA) REVISA, CNB_ITEM, CNB_SLDMED,CNB_PRODUT, CNB_VLUNIT ,CNB_QTDMED"
		cQryCNB += " FROM "
		cQryCNB += " 	" + RetSQLName("CNB")
		cQryCNB += " WHERE D_E_L_E_T_ = ' ' "
		cQryCNB += " 	AND CNB_CONTRA = '" + aDados[i, 02] + "' "
		cQryCNB += " 	AND CNB_PRODUT IN ('SERVR0007','SERVR0008')"
		cQryCNB += " 	AND CNB_ITMDST = ' ' "
		cQryCNB += " 	AND CNB_SLDMED > 0 "
		cQryCNB += " 	AND CNB_REVISA = '"+ CN9->CN9_REVISA +"'"
		cQryCNB += " GROUP BY CNB_CONTRA, CNB_NUMERO, CNB_ITEM, CNB_PRODUT, CNB_VLUNIT ,CNB_SLDMED,CNB_QTDMED"
		...
		If TMPCNB->( !Eof() ) 
				...
				oModel:LOADVALUE( 'CNBDETAIL'	, 'CNB_QUANT'   , TMPCNB->CNB_QTDMED)
				TMPCNB->( DBSKIP() ) 11
