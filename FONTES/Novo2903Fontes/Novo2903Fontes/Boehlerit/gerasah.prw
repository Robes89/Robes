...
USER FUNCTION gerasah()

	SetPrvt("CPERG,NOPC,CCADASTRO,aRegs")
	Private nOpc      := 0
	Private cCadastro := "Exportação de dados - Unidade de Medidas"
	Private aSay      := {}
	Private aButton   := {}
	Private _PEDNUM  := ""
	Private _DIRTXT    := "UNIDMED.TXT"
    ...
    @50,08 Say "Unidades de Medidas (SAH). " SIZE 100,140
	@70,08 GET _DIRTXT pict "@x" SIZE 120,100
	@080,140 BMPBUTTON TYPE 1 ACTION Close(oDlg)
	ACTIVATE MSDIALOG oDlg CENTERED

	aAdd( aSay, "Esta Rotina Irá Gerar o Arq. "+ALLTRIM(_DIRTXT)+" da Tabela: Unidades de Medidas." )
...
//    cLin := PADR(SAH->AH_UNIMED,2)+PADR(SAH->AH_DESCPO,10)
//    cLin := cLin + subs(DTOS(SC7->C7_EMISSAO),7,2)+"/"+subs(DTOS(SC7->C7_EMISSAO),5,2)+"/"+subs(DTOS(SC7->C7_EMISSAO),1,4)+subs(DTOS(SC7->C7_DATPRF),7,2)+"/"+subs(DTOS(SC7->C7_DATPRF),5,2)+"/"+subs(DTOS(SC7->C7_DATPRF),1,4)+Transform(SC7->C7_PRECO,"@E 999999999.9999")+Transform(SC7->C7_QUANT,"@E 999999999.99")
//    cLin := cLin + mv_par06

    cLin := PADR(SAH->AH_UNIMED,20)+PADR(SAH->AH_DESCPO,100)
    cLin := cLin + mv_par06
