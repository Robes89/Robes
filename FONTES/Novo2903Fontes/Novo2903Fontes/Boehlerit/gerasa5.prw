...
USER FUNCTION gerasa5()
	...
	Private nOpc      := 0
	Private cCadastro := "Exportação de dados - Fornecedores X Materiais"
	Private aSay      := {}
	Private aButton   := {}
	Private _PEDNUM  := ""
	Private _DIRTXT    := "FORNMAT.TXT"
    ...
    @50,08 Say "Fornecedores X Materiais (SA5). " SIZE 100,140
	@70,08 GET _DIRTXT pict "@x" SIZE 120,100
	@080,140 BMPBUTTON TYPE 1 ACTION Close(oDlg)
	ACTIVATE MSDIALOG oDlg CENTERED

	aAdd( aSay, "Esta Rotina Irá Gerar o Arq. "+ALLTRIM(_DIRTXT)+" da Tabela: Fornecedores X Materiais." )
...
While !EOF()
	...
	dbSelectArea("SE4")
	dbSeek(xFilial("SE4")+SA5->A5_COND01)

    cLin := PADR(SA5->A5_FORNECE,20)+PADR(SA5->A5_PRODUTO,20)+PADR(SA5->A5_CODPRF,20)
    cLin := cLin + PADR(SE4->E4_DESCRI,60)
    IF YEAR(SA5->A5_DTCOM01) = YEAR(SA5->A5_DTCOM02)
    	_wDias := SA5->A5_DTCOM02 - SA5->A5_DTCOM01
	    cLin := cLin + PADR(STR(_wDias),20)
	ELSEIF YEAR(SA5->A5_DTCOM01) > YEAR(SA5->A5_DTCOM02)
		if SA5->A5_DTCOM02 <> CTOD("  /  /  ")
	    	_wDias := SA5->A5_DTCOM01 - SA5->A5_DTCOM02
		else
			_wDias := 0
		endif
	    cLin := cLin + PADR(STR(_wDias),20)
	ENDIF
    cLin := cLin + PADR(STR(SA5->A5_PRECO01),20)
    cLin := cLin + mv_par06
