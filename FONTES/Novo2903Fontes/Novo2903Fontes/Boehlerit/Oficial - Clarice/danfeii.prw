...
#DEFINE MAXCODPRD  080					// Máximo de caracteres do codigo de produtos/servicos conforme o tamanho do quadro "Cod. prod"
...
Private lConsig	 := .F. //CUSTOMIZACAO - Indica se é venda por consignacao mercantil e subtraio o valor do IPI da impressão das despesas acessorias. Solicitado Regina 27-11-13. Esta adequacao é temporaria e foi aberto chamado na TOTVS
...
Private _nTotIpi := _nVSemIPI	:= 0 //CUSTOMIZACAO - Valor do ipi que nao deve ser impresso no total da nota caso tenha item com CSTIPI = 99 especifico para venda consiganado. Tanimoto 05-03-12
...
If ( MV_PAR04 == 1 )
	...
	If MsSeek(xFilial("SF1")+aNota[5]+aNota[4]+aNota[6]+aNota[7]) .And. SF1->(FieldPos("F1_FIMP"))<>0
		If SF1->F1_TIPO <> "D"
		_nTotIpi := Val(oTotal:_ICMSTOT:_vIPI:TEXT) //CUSTOMIZACAO - Guardo total do IPI para descontar no final para venda consignada. Tanimoto 05.03.12
	...
	If ValAtrib("oImposto:_Imposto")<>"U"
		...
		If ValAtrib("oImposto:_Imposto:_IPI")<>"U"
			If ValAtrib("oImposto:_Imposto:_IPI:_IPITrib:_vIPI:TEXT")<>"U"
		       If (oImposto:_Imposto:_IPI:_IPITrib:_CST:TEXT)=="99" //CUSTOMIZACAO - Tratamento especifico para LMT E FETTE para nao imprimir o IPI qdo CSTIPI for 99 de venda consignada. Tanimoto 05-03-12
					_nVSemIPI 	:= _nVSemIPI + Val(oImposto:_Imposto:_IPI:_IPITrib:_vIPI:TEXT)
				  	nValIPI 	:= 0
			  	Else
...
If _nTotIpi>0 //CUSTOMIZACAO - Imprime valor do IPI em informacoes coplementares quando venda de consignado com IPI. Tanimoto 05/03/12
	cAux := "Valor do IPI R$ "+Alltrim(Transform(_nTotIPI,"@ze 999,999.99"))
	While !Empty(cAux)
		aadd(aMensagem,{ SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN) - 1, MAXMENLIN)), .F. } )
		cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN), MAXMENLIN) + 1)
	EndDo
...
If nfolha==1
	If lMv_Logod
		oDanfe:SayBitmap(045,001,cLogoD,090,090)
	Else
...
//Calculo do imposto
//CUSTOMIZACAO - Tanimoto 27-11-13. Forco o valor de outras despesas que tem o valor do IPI como zero para nao destacar na venda consignacao mercantil. 
//Solicitado Regina.Esta adequacao e temporaria foi aberto chamado na totvs sobre este assunto
If lConsig
	aTotais[09] := Transform(Val("0"),"@ze 9,999,999,999,999.99")
EndIf																							  
...
oDanfe:Say(349+nAjustImp,420,Transform(_nTotIpi-_nVSemIPI,"@ze 9,999,999,999,999.99"),oFont08:oFont) //CUSTOMIZACAO - Subtrai valor do IPI que nao deve ser impresso ref ao produtos com CSTIPI=99 de venda consignado. Tanimoto 05/03/12
...
For nY := 1 To nLenItens
	...
	If lflag
		If nL > nMaxItemP2
			...
			If lMv_Logod
				oDanfe:SayBitmap(003+nLinhavers,001,cLogoD,090,090)
			Else
				oDanfe:SayBitmap(003+nLinhavers,001,cLogo,090,090)
			EndIf
	...
	If	nL > Iif( (nfolha-1)%2==0 .And. MV_PAR05==1,MAXITEMP3,nMaxItemP2)
		...
		If lMv_Logod
			oDanfe:SayBitmap(003,001,cLogoD,090,090)
		Else
			oDanfe:SayBitmap(003,001,cLogo,090,090)
		EndIf
...
Static Function RetTamCol(aCabec, aValores, oPrinter, oFontCabec, oFont)
	...
	*/		
	...
	aTamCol[1]:=80    ////  customizado
	aTamCol[2]:=163    ////  customizado
