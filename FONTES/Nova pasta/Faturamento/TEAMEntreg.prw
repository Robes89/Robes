...
Esta função tem a intenção de ajusatar o vencimento das parcelas de acordo com
...
IF ApMsgYesNo("Olá, apartir deste momento o sistema ira atualizar as parcelas existentes";
	+" no contas a reber de acorodo com a regar do clietne. Se deseja continuar";
	+" clique em 'SIM' caso contrario clique em 'Não' para finalizar  ", "Team Tex")
	
	DbSelectArea("SE1")
	SE1->(DbSetOrder(2))//E1_FILIAL+E1_CLIENTE+E1_LOJA+E1_PREFIXO+E1_NUM+E1_PARCELA+E1_TIPO
	
	IF SE1->(DbSeek(cKeySE1)) //Alteração deve ter o registro SE1 posicionado
		...
		WHILE SE1->(E1_FILIAL+E1_CLIENTE+E1_LOJA+E1_PREFIXO+E1_NUM) == cKeySE1			
			...
			WHILE SE1->(E1_FILIAL+E1_CLIENTE+E1_LOJA+E1_PREFIXO+E1_NUM) == cKeySE1
				...
				IF !EMPTY(SE1->E1_BAIXA) 	
					Alert("O título informado não poderá ter a data de vencimento alterada pois já se encontra baixado.")
                	Return()
				ENDIF
				
				IF SE1->E1_PARCELA $ 'A |  '
					dVencto:=DaySum( _XDTENT, 91)//1º Vencimento 90 Dias + 1 apos entrega.
					NovaData(cNreduz,dVencto)
				ELSE
					dVencto	:= DaySum(dVencto,30)//Passo para o proximo mês
					NovaData(cNreduz,dVencto)
				ENDIF
			...
			//MsExecAuto( { |a,b, c, d,e, f, g | FINA040(a,b, c, d,e, f, g)} , a  tit, 4,,,,, ) // 3 - Inclusao, 4 - Alteração, 5 - Exclusão
			MsExecAuto( { |a,b, c, d,e, f, g | FINA040(a,b, c, d,e, f, g)} , a  tit, 4,,,,, ) // 3 - Inclusao, 4 - Alteração, 5 - Exclusão
			*/
			SE1->(DbSkip())
		ENDDO
		...
		ELSE                  
			ApMsgInfo("Título(s) alterado(s) com sucesso!","Team Tex" )
			
		ENDIF
	ELSE
		ApMsgInfo("Não encontrou o SE1")
		Return()
...
IF "WMS" $ cNreduz .OR. "WMB" $ cNreduz .OR. "BOMPRECO" $ cNreduz
	IF 	Day(dVencto) <= 14 //Se o dia do mes for menor que 14 o vencimento vai para o dia 14 do mes atual se não para o dia 14 do proximo mês.
...
ELSEIF "CARREFOUR" $ cNreduz
	IF 	Day(dVencto) <= 10 //Se o dia do mes for menos que 10 o vencimento vai para o dia 10 do mes atual se não para o dia 10 do proximo mês.
...
ELSEIF "HAVAN" $ cNreduz
	...
	//Se o dia do mes for maior que 15 o vencimento vai para o ultimo dia do mês. 
	IF 	Day(dVencto) <= 15 
		dVencto := DaySum(dVencto,(15 - Day(dVencto)))//Ajusto para a data para dia 15
		SE1->E1_VENCTO := dVencto
	ELSE                                                                          
		dVencto := LastDate(dVencto)//Ajusto para o ultimo dia do mês.
		SE1->E1_VENCTO := dVencto
	ENDIF	
