            ...
            If FieldPos("F1_TRANSP") > 0 .And. !Empty(SF1->F1_TRANSP)
                ...
                aadd(aTransp,SA4->A4_EMAIL  )
                    If !Empty(SF1->F1_PLACA)
                        ...
                        aadd(aVeiculo,"")//RNTC
                    EndIf       
                //Campos customizados, buscam informações digitadas na nota ESPECIFICO TEAMTEX
...
If !Empty(aNota)
    ...
    Next nX
    cString += NfeTotal(aTotal,aRetido,aICMS,aICMSST,lIcmDevol,cVerAmb,aISSQN,nVicmsDeson,aNota,nVIcmDif,aAgrPis,aAgrCofins,aProd)
...
cString += '<enderEmit>' //Thiago SM0->M0_COMPCOB colocado complemento de endereço                   
cString += '<Lgr>'+IIF(!lEndFis,ConvType(FisGetEnd(SM0->M0_ENDCOB,SM0->M0_ESTCOB)[1])+" "+SM0->M0_COMPCOB,ConvType(FisGetEnd(SM0->M0_ENDENT,SM0->M0_ESTENT)[1]))+'</Lgr>'
...
//Alteracao TRIYO - Thiago Menegocci - Incluir 
//Inclusao de Cod EAN na descrição do produto 
If SA1->A1_CLIEAN == 1 
	cString += '<Prod>'+ConvType(aProd[04],120)+" - "+cEan+'</Prod>' 
Else 
...
Static Function NfeTotal(aTotal,aRet,aICMS,aICMSST,lIcmDevol,cVerAmb,aISSQN,nVicmsDeson,aNota,nVIcmDif,aAgrPis,aAgrCofins,aProd)
...
//cString += '<vNF>'+ConvType(_xntot+aTotal[03],15,2)+'</vNF>' // processo team tex
